...
let realspeed = 0.004*movingSpeed;
...
p.draw = function() {
  ...
  if (first === 0) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      colorsArray[i] = changeColors!=0?p.random(changeColors):p.random(colors); // Randomly assign a color from the colors array
    }
    first = 2;
  } else {
    mx += (adjustedX - mx) / 4;
    ...
    for (let i = 0; i < n; i++) {
      ...
      p.ellipse(x * p.width * (p.cos(r) + p.sin(r)),
              ...
              ss[i], ss[i]);
      rs[i] += (x / (x + 1) + 0 * close / 4) * realspeed / xs[i] * (20 / p.frameRate());
      p.pop();
    }
  }
